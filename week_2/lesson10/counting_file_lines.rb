...
if File.exist?(file_path)
  ...
  # Другой (более трудный) способ — просто вызвать у объекта file метод
  ...
  puts "Пустых строк: #{lines.select { |str | str == "\n" && str.length == 1 }.length}"
  "Последние 5 строк файла: #{
    lines.each.with_index do |str, idx |
      if idx > 4
        puts "Это строка #{idx} с конца: #{str}"
      end
    end
  }"
